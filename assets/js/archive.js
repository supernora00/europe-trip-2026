(() => {
  const STORAGE_KEY = 'nora-journey--europe-2026';
  ...
  async function loadData() {
    ...
    if (saved) {
      try { return JSON.parse(saved); } catch (error) { console.warn(' data reset:', error); }
    }
    if (window.NORA_ARCHIVE_DATA) return JSON.parse(JSON.stringify(window.NORA_ARCHIVE_DATA));
    throw new Error('无法读取  数据');
  }
  ...
  function renderList() {
    ...
    list.innerHTML = state.data.days.map(item => `
      <button class="-day-row ${item.day === state.selectedDay ? 'active' : ''}" data-day="${item.day}" type="button">
        <span class="-day-number">${String(item.day).padStart(2,'0')}</span>
        <span class="-day-copy"><strong>${escapeHTML(item.city)}</strong><small>${formatDate(item.date)} · ${escapeHTML(item.title)}</small></span>
  ...
  function renderViewer(item) {
    ...
    return `
      ...
      <div class="-entry-section"><span class="-label">Journal</span><div class="journal-copy ${item.journal ? '' : 'empty'}">${item.journal ? paragraphs(item.journal) : '旅程中或回来后，在这里写下当天最想记住的事。'}</div></div>
      <div class="-entry-grid">
        <div class="-entry-section"><span class="-label">Places</span><div class="place-chips">${places}</div></div>
        <div class="-entry-section"><span class="-label">Expense</span><strong class="expense-value">${escapeHTML(item.expense || '—')}</strong></div>
      </div>
      <div class="-actions"><button class="btn primary" type="button" data-edit>编辑这一天</button><button class="btn" type="button" data-export>导出  JSON</button></div>`;
  }

  function renderEditor(item) {
    return `
      <form class="-editor" data-editor>
        <div class="-editor-head"><div><div class="eyebrow">Editing Day ${item.day}</div><h2>${escapeHTML(item.city)}</h2></div><button class="close-editor" type="button" data-cancel>×</button></div>
        ...
        <div class="-actions"><button class="btn primary" type="submit">保存</button><button class="btn" type="button" data-cancel>取消</button></div>
      </form>`;
  }
  ...
  function exportJSON() {
    ...
    link.href = url; link.download = 'nora-europe-2026-.json'; link.click();
  ...
  loadData().then(data => { state.data = data; render(); }).catch(error => {
    $('[data--entry]').innerHTML = `<p class="-error">${escapeHTML(error.message)}。请通过 GitHub Pages 或本地服务器打开网站。</p>`;
  });
})();
